...
variable "firewall_manager_administrator_role_arn" {
  type        = string
  description = "The ARN of the role you assume for destroying the component - must be the AWS Firewall Manager administrator account (contains admin_account_ids) "
  default     = null
}

variable "is_destroy" {
  type        = bool
  description = "Set to `true` to destroy the component"
  default     = false
}

variable "admin_account_enabled" {
  type        = bool
  description = "Resource for aws_fms_admin_account is enabled and will be created. If set to `false`,  will be destroyed"
  default     = true
}
...
variable "firehose_arn" {
  type        = string
  description = "Kinesis Firehose ARN to create a Kinesis Firehose destination for WAF_V2 Rules. Conflicts with `firehose_enabled`"
  default     = null
}
