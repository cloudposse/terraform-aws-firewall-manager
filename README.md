...
<br>policy\_data:<br>  automatic\_response\_status:<br>    Status of shield automatic response.<br>    Possible values: ENABLED\|IGNORED\|DISABLED.<br>    Default is IGNORED.<br>  automatic\_response\_action:<br>    The automatic response action.<br>    Possible values: BLOCK\|COUNT.<br>    Default is null.<br>  override\_customer\_webacl\_classic:<br>    Whether to replace AWS WAF Classic web ACLs with this policy's AWS WAF v2 web ACLs where possible.<br> | `list(any)` | `[]` | no |
