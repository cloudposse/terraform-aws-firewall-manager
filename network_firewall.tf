...
resource "aws_fms_policy" "network_firewall" {
  ...
  security_service_policy_data {
    ...
    managed_service_data = jsonencode({
      ...
      networkFirewallStatefulRuleGroupReferences     = lookup(each.value.policy_data, "stateful_rule_group_references", [])
      networkFirewallOrchestrationConfig = {
        ...
        allowedIPV4CidrList          = lookup(each.value.policy_data.orchestration_config, "allowed_ipv4_cidr_list", [])
      }
      }
    )
  }
