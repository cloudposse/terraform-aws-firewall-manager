...
resource "aws_fms_policy" "security_groups_content_audit" {
  ...
  security_service_policy_data {
    ...
    managed_service_data = jsonencode({
      type                = "SECURITY_GROUPS_CONTENT_AUDIT"
      securityGroupAction = upper(each.value.managed_service_data.security_group_action)
      securityGroups      = flatten([for sg in tolist(each.value.managed_service_data.security_groups) : { id = sg }])
    })
  }
}
